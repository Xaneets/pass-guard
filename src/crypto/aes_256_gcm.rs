...
impl Aes256Gcm {
    pub fn encrypt(mut data: Vec<u8>, hash: [u8; 32], nonce_value: NonceSeq) -> Result<(Vec<u8>, Tag, NonceSeq), Unspecified> {
    ...
    pub fn decrypt(data: Vec<u8>, hash: [u8; 32], nonce_value: NonceSeq, tag: Tag) -> Result<(Vec<u8>, Tag, NonceSeq), Unspecified> {
        let unbound_key = UnboundKey::new(&AES_256_GCM, &hash)?;
        let aad = Aad::empty();
